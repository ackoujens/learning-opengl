...
public:
    ...
    void startup() {
      ...
      static const GLchar * geometryShaderSource[] =
      {
        ...
        // indicates that the geometry shader expects to see triangles as input
        "layout (triangles) in; \n"

        // tells OpenGL that the geometry shader will produce points
        // and that the max number of points that each shader will produce will be 3
        "layout (points, max_vertices = 3) out; \n"
        " \n"
        "void main(void) { \n"
        "   int i; \n"
        "   for (i = 0; i < gl_in.length(); i++) { \n"
                // set position of the resulting vertex
        "       gl_Position = gl_in[i].gl_Position; \n"

                // produces vertex at the output of the geometry shader
        "       EmitVertex(); \n"

                // geometry shaders automatically call EndPrimitive() at the
                // end of your shader -> not necessary calling this
        "   } \n"
        "} \n"
      };
